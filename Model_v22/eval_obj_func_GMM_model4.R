...
eval_covariates_delta_reg <- function(deltain,theta1,wdcMerged,points) {
  ...
  length_levels <- c(length(levels(wdcMerged$week_fac)),length(levels(wdcMerged$Conditions)), length(levels(wdcMerged$Temperature_group)),
                     length(levels(wdcMerged$Humidity_high)), length(levels(wdcMerged$Wind.Speed_high)))
  
  reg_terms <- c("week_fac", "Conditions", "Temperature_group", "Humidity_high", "Wind.Speed_high")[which(length_levels>1)]
  if(length(reg_terms)) {
  ...
  if(length(which(colnames(Xbase) %like% "tract_tw_fac")) != length(levels(wdcMerged$tract_tw_fac))) {
    stop("in eval_error_xi_sl_model4 all tract_tw_fac not included")      
  }
  if(length(which(colnames(Xbase) %like% "week_fac")) != length(levels(wdcMerged$week_fac))-1) {
    stop("in eval_error_xi_sl_model4 one week_fac number not automatically removed")      
  }
...
eval_error_xi_model4 <- function(theta1,deltain,wdcMerged,points) {
  ...
  covariates_delta_reg <- eval_covariates_delta_reg(deltain,theta1,wdcMerged,points)
  X <- covariates_delta_reg$X
  Z <- covariates_delta_reg$Z
  ...
  print("kappa theta2 inv step: ")
  print(kappa(t(X) %*% Z_weighted %*% A_N %*% t(Z_weighted) %*% X))
...
eval_obj_GMM_model4_obj <- function(params, wdcMerged, points, length_theta) {    
  ...
  if(print_iter_values) {
    print("params: theta, summary of delta")
    print(theta)
    print(as.numeric(summary(deltain_stin)))
    print("")    
  }
...
eval_obj_GMM_model4 <- function(theta1, deltain, wdcMerged, points) {  
  ptm <- proc.time()
...
#   print(paste(theta1))
  ...
  tryCatch({
    ...
    gmm_grad_theta <- rep(0,length(theta1))
