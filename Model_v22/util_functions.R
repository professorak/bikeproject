...
gen_sto_state_hyperlocal_char <- function(so_vec,local_stations,hyperlocal_range) {
  so_state_local_vec <- rep(NA,length(local_stations))
  #for each of the local_stations entry, generate the projection from so_vec and create a char from it
  for(i in 1:length(local_stations)) {
    indexvec <- as.integer(strsplit(as.character(local_stations[i]),split="_")[[1]])
    so_state_local_vec[i] <- paste(as.numeric(c(so_vec[indexvec[c(1:min(hyperlocal_range,length(indexvec)))]],
                       rep(0,pos(length(indexvec)-hyperlocal_range)))),collapse="_")
  }
  return(so_state_local_vec)
}

# gen_sto_state_nonhyperlocal_char <- function(so_vec,stations_hyperlocal) {
#   so_state_local_vec <- rep(NA,length(stations_hyperlocal))
#   #for each of the local_stations entry, generate the projection from so_vec and create a char from it
#   for(i in 1:length(stations_hyperlocal)) {
#     indexvec <- as.integer(strsplit(as.character(stations_hyperlocal[i]),split="_")[[1]])
#     so_state_local_vec[i] <- paste(rep(0,length(indexvec)),collapse="_")
#   }
#   return(so_state_local_vec)
# }
...
get_grad_total_density  <- function(x0_in, wdcMerged, points) {
  ...
  grad <- c()
  for(tw_in in tw_list) {
    grad <- rbind(grad, c(rep(0,nonden_ceoflength),colSums(get_grad_density_vec(x0_in, tw_in))))
  }
...
get_density_vec <- function(x0_in, tw_in) {  
  return(get_points_density(points, x0_in, tw_in))
}
...
#replacement for get_points_density_grad_bus_col
get_points_density_grad_addden_cols <- function(points_in, tw_in) {
  density <- cbind((points_in$type==3),(points_in$type==4)) * points_in$weight    
  return(density)
}
...
# get_theta1fromtheta <- function(theta) {
#   return(c(theta[1],0,theta[-1]))
# }

trim_serv <- function(x, range) {
  x[which(x<range[1] | x>=range[2])] <- 0
  return(x)
}

trim_serv_frac <- function(x, range) {
  min_x <- min(x)
  len_x <- max(x)-min(x)
  range_x <- min(x) + range*len_x
  x[which(x<range_x[1] | x>=range_x[2])] <- 0
  return(x)
}


scalecols <- function(X, scalecols_X) {
  return(t(t(X)/scalecols_X))
}
...
normalize_vec <- function(x) {
  ...
  return((x-mean(x))/sd(x)/4)
}

convert_sto_state_to_zero <- function(str,splitchar="_"){  
  return(paste(rep(0,length(strsplit(as.character(str)
                                     , split=splitchar)[[1]])),collapse="_"))
}
