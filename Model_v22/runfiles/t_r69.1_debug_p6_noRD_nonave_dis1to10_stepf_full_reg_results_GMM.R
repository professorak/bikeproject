...
source("GetDataGooglePlaces_aggmonths_norealloc_stkoutthresh_five.R")
v0_vec <- c(0)
...
list_tourist_vars <- c("touristlocs_den_a","touristlocs_den_b","touristlocs_den_c")
wdcMerged[,list_metro_vars] <- wdcMerged_newvars[,list_metro_vars]
wdcMerged[,list_tourist_vars] <- wdcMerged_newvars[,list_tourist_vars]
...
source("eval_covariates_delta_reg_densitycols_setb35_wmetro_off.R") 
...
theta1 <- c(-4.22630742730619, 0, -6.57217747330829,
  0, 0.998281420902327, 0.607202817591749, 5.69865104826994, 0, 0, 0, 0, 0, 0, 26.2774806507998)
...
theta1_case2 <- c(-2.76055939246944, 0.000256961963190648, -23.4934317484546,
  0, 0.207972646679347, 0.243387248189416, 0, 0, 0, 0, 0, 0, 0, 4.08476395803412)
...
for(i in c(4:length(theta1_case2))) {
  theta_division_case2 <- rep(0,length(theta1_case2))
  theta_division_case2[c(1:3)] <-  theta1_case2[c(1:3)]
  theta_division_case2[i] <- theta1_case2[i]
  demand_case2_i <- eval_lambda_full(deltain_stin_case2, theta_division_case2,wdcMerged, points)
...
##################
#case3 with just the intercept

theta1_case3 <- c(-8.74463799748461, 0.000673370992741172, -10.4642375185142,
                  0,0,0.1817587,0.00000000,0,0,
                  0,0,0,0,0)

get_total_density(theta1_case3,wdcMerged,points)
delta_all_case3 <- compute_delta_list_cntrt_map_new(theta1_case3,wdcMerged, points)
deltain_stin_case3 <- delta_all_case3[which(wdcMerged$stocked_out==F)]

eta_case3 <- rep(0,length_eta)
eta_case3 <- c(eval_constraints_moment_conditions(deltain_stin_case3, 
                                                  theta1_case3, eta_case3, wdcMerged, points))
params_case3 <- c(theta1_case3,deltain_stin_case3,eta_case3)


obj_case3 <- eval_obj_GMM_model5_obj(params_case3, wdcMerged, points, length(theta1), length(deltain_stin), length_eta)

xi_case3 <- eval_error_xi_model5 (deltain_stin_case3, 
                                  theta1_case3, wdcMerged,points)$xi
plot(xi_case3, type="l",ylim=c(-1.1,1.1))
######################################################################
######################################################################
#comparing
...
plot(xi, type="l",ylim=c(-10,10))
plot(xi^2, type="l")
plot(deltain_stin, type="l")
plot(xi_case2, type="l",ylim=c(-1.1,1.1))
plot(xi_case2^2, type="l")
plot(deltain_stin_case2, type="l")
plot(xi_case2-xi, type="l")
plot(xi_case2^2-xi^2, type="l")
...
top_xi_idx <- which(xi_case2^2-xi^2>0.1)
xi[top_xi_idx]
...
for(i in 1:nrow(outliers_2)) {
  ...
  vals <- c()
  vals <- c(vals,c(xi[outliers_2$station_id_index[i]],xi_case2[outliers_2$station_id_index[i]]))
  vals <- c(vals,outliers_2$out_dem_mean[i])
  ...
  print(cbind(c("xi","xi_case2","out_dem_mean val"
                ,"percentile out_dem_mean","metro_den_on_1","metro_den_on_2","googleplaces_food_den_1","googleplaces_food_den_2",
                "googleplaces_grocery_den_1"
