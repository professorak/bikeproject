...
sink("rb72.1_metrodummies_logmetrocorrected_ls_googlevar4_1:20_full_nldis_pr6.txt",split=T)
...
theta1 <- c(-2.309775e+00,1.839464e-03, -2.500129e+01,0,
            2.101448e-01,2.411672e-01,0.000000e+00,c(0,0,0,0.1,0),0.000000e+00,4.587059e+00,0.1,0)

active_coef <- c(1:length(theta1))[-c(density_metro_evening_col,density_google_places_points_cols[-4],
                                      density_add_den_cols[c(1,4)])]
active_coef_nointercept <- c(1:length(theta1))[-c(density_intercept_col,density_metro_evening_col,density_google_places_points_cols[-4]
                                                  ,density_add_den_cols[c(1,4)])]
...
ub = c(c(300,100,300),
       1000000,rep(1000000,2),0,c(0,0,0,1000000,0),0,1000000,1000000,0,
       rep(100, length_delta), rep(1.0e19,length_eta))
