...
sink("r81.7.1_noRD_googlevar1,4,5_tw1to5_hyperlocal_majoritystates_preweatherreg_metrodummies_moments_1:20_full_nldis_pr6.txt",split=T)
...
wdcMerged <- subset(wdcMerged, tw %in% tw_set)
wdcMerged <- droplevels(wdcMerged)
user_serv_lvl <- subset(user_serv_lvl, tw %in% tw_set)
...
source("eval_covariates_delta_reg_densitycols_setb49.6_googlevar1,4,5_wservlvl_metrooff_metrodummies.R")  
...
theta1 <- c(-1.00421741890612, 0, -52.3763133651414, 0, 0.802624863248286, 
            0.200009340943852, 1.2148213818213, 
            0.1, 0, 0, 0.1, 0.1, 
            0, 6.57149540141334, 0, 0)

metro_on_idx <- which(colnames_theta1 %in% c("density metro","density metro dummy"))
metro_off_idx <- which(colnames_theta1 %in% c("density metro evening","density metro evening dummy"))
metro_on_theta1 <- theta1[metro_on_idx]
metro_off_theta1 <- theta1[metro_off_idx]
theta1[c(metro_on_idx,metro_off_idx)] <- 0

active_coef <- c(1:length(theta1))[-c(2,density_google_places_theta_cols[-c(1,4,5)],
                                      density_ridership_col,density_add_den_cols[1],metro_on_idx,metro_off_idx)]
active_coef_nointercept <- c(1:length(theta1))[-c(2,density_intercept_col,
                                                  density_google_places_theta_cols[-c(1,4,5)],density_ridership_col,density_add_den_cols[1],
                                                  metro_on_idx,metro_off_idx)]
...
ub = c(c(50,0,50),
       0,rep(1000000,2),1000000,c(1000000,0,0,1000000,1000000),0,1000000,1000000,1000000,
       rep(10, length_delta), rep(1.0e19,length_eta))
ub[c(metro_on_idx,metro_off_idx)] <- 0
...
if(length(intersect(c(1:5),tw_set))>0) {
  #include metro_on_idx
  active_coef <- sort(c(active_coef, metro_on_idx))
  active_coef_nointercept <- sort(c(active_coef_nointercept, metro_on_idx))
  ub[metro_on_idx] <- 10000
  theta1[metro_on_idx] <- metro_on_theta1
}
if(length(intersect(c(0),tw_set))>0) {
  #include metro_off_idx
  active_coef <- sort(c(active_coef, metro_off_idx))
  active_coef_nointercept <- sort(c(active_coef_nointercept, metro_off_idx))
  ub[metro_off_idx] <- 10000
  theta1[metro_off_idx] <- metro_off_theta1
}

if(any(params>ub) | any(params<lb)) {stop("params not within lb-ub")}
