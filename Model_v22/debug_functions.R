...
assign_log_metro_traffic_corrected <- function(points) {
  #log of absolute metro traffic
  points_metro_idx <- which(points$type==2)
  
  metrotrafficloc_file_name <- paste0(dropbox_dir,
                                      "/../VelibData/ParisData/ParisMetroData/metro_stations_data_curated_log.csv") 
  ...
  metro_traffic_locations <- metro_traffic_locations[which(metro_traffic_locations$lat %in% points$lat[points_metro_idx] &
                                                             metro_traffic_locations$lon %in% points$lon[points_metro_idx]),]
  metro_traffic_locations <- metro_traffic_locations[order(metro_traffic_locations$lat, 
                                                           metro_traffic_locations$lon),]
  identical(points$lat[points_metro_idx], metro_traffic_locations$lat)
  identical(points$lon[points_metro_idx], metro_traffic_locations$lon)
  
  points$weight[points_metro_idx] <- metro_traffic_locations$density
