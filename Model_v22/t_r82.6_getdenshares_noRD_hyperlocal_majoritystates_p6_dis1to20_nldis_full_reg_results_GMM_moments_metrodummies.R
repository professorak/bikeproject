...
sink("r82.6_getdenshares_noRD_hyperlocal_majoritystates_metrodummies_moments_1:20_full_nldis_pr6.txt",split=T)
...
source("GetDataGooglePlaces_20dis_aggmonth_hyperlocalstate_majoritystates_moreinstr_norealloc_stkoutthresh_five_averaged_pr6.R")
v0_vec <- c(0)
...
theta1 <- c(-6.492209e-01,0.000000e+00, -4.831875e+01,3.255039e-04,1.014554e+00,2.397067e-01,1.399492e+00,
  0.000000e+00,0.000000e+00,0.000000e+00,0.000000e+00,0.000000e+00,0.000000e+00,1.538374e+00, 0, 0)
...
#set params
theta1 <- c(-4.042505595,0.000000000,-14.758737137,0.004049788,2.651335342,0.132942080,1.393720150,
  0.000000000,0.000000000,0.000000000,0.000000000,0.000000000,0.000000000,108.777501515,3.793228323,2.600441064)
delta_all <- compute_delta_list_cntrt_map_new(theta1,wdcMerged, points, 1e-6, 1e-6)
deltain_stin <- delta_all[which(wdcMerged$stocked_out==F)]
eta <- rep(0,length_eta)
eta <- c(eval_constraints_moment_conditions(deltain_stin, theta1, eta, wdcMerged, points))
