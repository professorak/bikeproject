...
get_weather_scale <- function(wdcMerged) {
  ...
  wdcMerged$time_halfhour_int_fac <- as.ff(as.factor(wdcMerged$time_halfhour_int[]))
  
  agg_out_dem = binned_sum(wdcMerged$out_dem,wdcMerged$time_halfhour_int_fac)  
  agg_in_dem = binned_sum(wdcMerged$in_dem,wdcMerged$time_halfhour_int_fac)
  if(!identical(rownames(agg_out_dem),rownames(agg_in_dem))) stop("in aggdata, aggregated data not in same order")
  agg_stockedout = binned_sum(wdcMerged$stocked_out,wdcMerged$time_halfhour_int_fac)
  agg_stockedfull = binned_sum(wdcMerged$stocked_full,wdcMerged$time_halfhour_int_fac)
  
  wdcMerged_weather_df <- data.frame(time_halfhour_int_fac=rownames(agg_out_dem),
  ...
  wdcMerged_weather_df$time_halfhour_int <- as.numeric(as.character(wdcMerged_weather_df$time_halfhour_int_fac))
  ...
  wdcMerged_weather_df <- merge(wdcMerged_weather_df,weather_data, by="time_halfhour_int")    
  
  #merge with wdcMerged reduced form to get tw corresponding to time_halfhour_int
  wdcMerged_weather_df <- wdcMerged_weather_df[order(wdcMerged_weather_df$time_halfhour_int),]
  time_halfhour_int_df <- wdcMerged[,c("time_halfhour_int","tw")]
  time_halfhour_int_df$dup <- duplicated(time_halfhour_int_df$time_halfhour_int)
  time_halfhour_int_df <- subset(time_halfhour_int_df, dup==F)
  time_halfhour_int_df <- time_halfhour_int_df[order(time_halfhour_int_df$time_halfhour_int),]
  if(!identical(wdcMerged_weather_df$time_halfhour_int,time_halfhour_int_df$time_halfhour_int)) 
    stop("wdcMerged_weather_df and time_halfhour_int_df dont match")
  time_halfhour_int_df$tw <- floor(time_halfhour_int_df$tw/4)
  #merge
  wdcMerged_weather_df <- merge(wdcMerged_weather_df,time_halfhour_int_df, by="time_halfhour_int")    
  
  wdcMerged_weather_df_tw_save <- wdcMerged_weather_df
  
  #run reg
  fit1 <- lm(log(out_dem_rate+1e-3) ~ factor(tw), data=wdcMerged_weather_df)
  summary(fit1)
  
  fit_out_dem <- lm(log(out_dem_rate+1e-3) ~ factor(tw) + Conditions + Temperature_group +
                      Humidity_high + Wind.Speed_high, data=wdcMerged_weather_df,
                    weights=non_stockedout_obs)
  print("summary(fit_out_dem)")
  print(summary(fit_out_dem))
  
  fit_in_dem <- lm(log(in_dem_rate+1e-3) ~ factor(tw) + Conditions + Temperature_group +
                  Humidity_high + Wind.Speed_high, data=wdcMerged_weather_df,
  ...
  outdem_weather_scale <- outdem_weather_scale - mean(outdem_weather_scale)
  ...
  indem_weather_scale <- indem_weather_scale - mean(indem_weather_scale)
  indem_weather_scale <- exp(indem_weather_scale)
  
  weather_scale_df <- data.frame(time_halfhour_int=wdcMerged_weather_predictdata_df$time_halfhour_int,
