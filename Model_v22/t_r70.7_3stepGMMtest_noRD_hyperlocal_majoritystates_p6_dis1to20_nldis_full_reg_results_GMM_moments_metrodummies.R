...
sink("r70.7_3stepGMMtest_noRD_hyperlocal_majoritystates_metrodummies_moments_1:20_full_nldis_pr6.txt",split=T)
...
theta1 <- c(-2.365283357,0,-48.16548004,0,0.767321134,0.247682417,1.175579531,
            0,0,0,0,0,0,6.220486591,0,0)
...
theta1 <- c(-2.22847843, 0, -20.08377898, 0.006113277, 2.10572608, 0.121733355, 1.613412983, 
          0, 0, 0, 0, 0, 0, 90.61208021, 3.685724337, 3.268954978)
delta_all <- compute_delta_list_cntrt_map_new(theta1,wdcMerged, points, 1e-6, 1e-6)
deltain_stin <- delta_all[which(wdcMerged$stocked_out==F)]
eta <- rep(0,length_eta)
eta <- c(eval_constraints_moment_conditions(deltain_stin, theta1, eta, wdcMerged, points))
