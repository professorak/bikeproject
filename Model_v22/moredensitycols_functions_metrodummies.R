get_points_density <- function(points_in, theta1, tw_in) {
  if(tw_in == 0) {
    density <- ((points_in$type==1)*theta1[density_ridership_col] + 
      ...
      (points_in$type==2)*theta1[density_add_den_cols[4]]
  } else{
    density <- ((points_in$type==1)*theta1[density_ridership_col] + 
                  (points_in$type==2)*theta1[density_metro_col]) *points_in$weight   + 
      (points_in$type==1)*theta1[density_intercept_col] +
      (points_in$type==2)*theta1[density_add_den_cols[3]]
  }
  density <- density + 
...
get_points_density_grad_addden_cols <- function(points_in, tw_in) {
  ...
  density <- cbind(density, (tw_in != 0)*(points_in$type==2),
                   (tw_in == 0)*(points_in$type==2))
  return(density)
}
