...
sink("r82.7.1_noRD_dummygooglevar1,4,5_hyperlocal_majoritystates_preweatherreg_metrodummies_moments_1:20_full_nldis_pr6.txt",split=T)
#_googlevar1=local_government_office
...
#dummify google vars
points$lodging <- as.numeric(points$lodging>0)
points$museum <- as.numeric(points$museum>0)
points$movie_theater <- as.numeric(points$movie_theater>0)
points$local_government_office <- as.numeric(points$local_government_office>0)
points$food <- as.numeric(points$food>0)
...
list_update_vars <- c("metro_den_on_1","metro_den_on_2","metro_den_off_1","metro_den_off_2",
                     ...
                     "googleplaces_den_1","googleplaces_den_2","googleplaces_den_3","googleplaces_den_4",
                     "log(googleplaces_den_3+1)","log(googleplaces_den_4+1)",
                     ...
                     "googleplaces_grocery_den_1","googleplaces_grocery_den_2","googleplaces_grocery_den_3",
                     "googleplaces_grocery_den_4","googleplaces_grocery_den_a","googleplaces_grocery_den_b",
                     "googleplaces_grocery_den_c","googleplaces_government_den_1","googleplaces_government_den_2",
                     "googleplaces_government_den_3","googleplaces_government_den_4","googleplaces_government_den_a",
                     "googleplaces_government_den_b","googleplaces_government_den_c","googleplaces_lodging_den_1",
                     "googleplaces_lodging_den_2","googleplaces_lodging_den_3","googleplaces_lodging_den_4",
                     "googleplaces_museum_den_1","googleplaces_museum_den_2","googleplaces_museum_den_3",
                     "googleplaces_museum_den_4","googleplaces_movie_theater_den_1","googleplaces_movie_theater_den_2",
                     "googleplaces_movie_theater_den_3","googleplaces_movie_theater_den_4")
wdcMerged[,list_update_vars] <- wdcMerged_newvars[,list_update_vars]
...
theta1 <- c(-1.00421741890612, 0, -52.3763133651414, 0.00180655817461775, 0.802624863248286, 
            0.200009340943852, 1.2148213818213, 
            0.1, 0, 0, 0.1, 0.1, 
            0, 6.57149540141334, 0, 0)


active_coef <- c(1:length(theta1))[-c(2,density_google_places_theta_cols[-c(1,4,5)],
                                      density_add_den_cols[1])]
active_coef_nointercept <- c(1:length(theta1))[-c(2,density_intercept_col,
                                                  density_google_places_theta_cols[-c(1,4,5)],density_add_den_cols[1])]
...
ub = c(c(50,0,50),
       1000000,rep(1000000,2),1000000,c(1000000,0,0,1000000,1000000),0,1000000,1000000,1000000,
...
serv_cols <- c(serv_cols, which(rownames(theta2) %like% "dis_center")) 
...
for(i in c(4:length(theta1))) {
  ...
  demand_i <- eval_lambda_full_weighted(deltain_stin, theta_division,wdcMerged, points)
  demand_division_df <- cbind(demand_division_df, demand_i)
}
colnames(demand_division_df) <- colnames_theta1[4:length(colnames_theta1)]
summary(rowSums(demand_division_df)-wdcMerged$out_dem_sum)
print(t(t(colSums(demand_division_df)))/sum(demand_division_df)*100)
#fraction of variance explained by density elements
demand_variance_explained_vec <- c()
for(i in c(4:length(theta1))) {  
  var_exaplined <- cov.wt(as.matrix(demand_division_df[,i-3]/wdcMerged$obs_weight), wt = wdcMerged$obs_weight)$cov/
    cov.wt(as.matrix(wdcMerged$out_dem_sum/wdcMerged$obs_weight), wt = wdcMerged$obs_weight)$cov
  demand_variance_explained_vec <- c(demand_variance_explained_vec,var_exaplined)
}
print(cbind(colnames_theta1[4:length(colnames_theta1)],",",demand_variance_explained_vec))

#alternative formulation just based on means and no weights
demand_variance_explained_vec <- c()
for(i in c(4:length(theta1))) {  
  var_exaplined <- var(demand_division_df[,i-3]/wdcMerged$obs_weight)/var(wdcMerged$out_dem_sum/wdcMerged$obs_weight)
  demand_variance_explained_vec <- c(demand_variance_explained_vec,var_exaplined)
}
print(cbind(colnames_theta1[4:length(colnames_theta1)],",",demand_variance_explained_vec))

#compute R2
#compute xi, substract from delta
#compute predicted demand and take weighted variance ratios
delta_no_xi <- deltain_stin - eval_xi$xi
predicted_lambda <- eval_lambda_full_weighted(delta_no_xi, theta1, wdcMerged, points)  
Rsquared <- cov.wt(as.matrix(predicted_lambda/wdcMerged$obs_weight), wt = wdcMerged$obs_weight)$cov/
  cov.wt(as.matrix(wdcMerged$out_dem_sum/wdcMerged$obs_weight), wt = wdcMerged$obs_weight)$cov
print(Rsquared)
##########
...
print(paste0(theta1_save_t_stat[-c(1:(nonden_ceoflength))],collapse = ", " ))
print("theta2 output: ")
print(theta2_save[serv_cols]/serv_lvl_scale)
print("theta2 t-stat:")
...
print(paste0(theta1_t_stat[-c(1:(nonden_ceoflength))],collapse = ", " ))
print("theta2 output: ")
print(theta2[serv_cols]/serv_lvl_scale)
print("theta2 t-stat:")
...
theta2coef_s1 <- theta2_save[serv_cols]/serv_lvl_scale
...
theta2coef <- theta2[serv_cols]/serv_lvl_scale
