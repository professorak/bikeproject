...
compute_delta_list_cntrt_map_new <- function(theta1, wdcMerged, points) {
  ...
  for(i in 1:length(tw_group_list)) {
    ...
    if(!is.null(x0_start)) {
      ...
      points_density <- ((points$type==1)*theta1[density_ridership_col] + 
                           (points$type==2)*theta1[density_metro_col]) *points$weight + 
                          (points$type==1)*theta1[density_intercept_col]
...
eval_share_log_list_new <- function(expdeltain, theta1, wdcMergedday, points, tw_groupin) {
  ...
  no_st <- max(wdcMergedday$station_id_index)
  lambda_list <- eval_lambda_delta_list_new(deltain, theta1, wdcMergedday, points, tw_groupin)
  lambda_t <- lambda_list[[1]]
...
#   dem_hat_T <- by(wdcMergedday$out_dem_sum, wdcMergedday[,"station_id_index"], FUN=sum)  
  ...
  return( list( "dem_T" = lambda_t,
                "dem_hat_T"  = wdcMergedday$out_dem_sum) )
