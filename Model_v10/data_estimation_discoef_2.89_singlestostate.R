...
point_range <- 3
no_bootstrap_runs <- 1
reg_runs_save_file <- "reg_runs_discoef_2.89_1state_mw6_bootstrap.txt"
...
#   filelist_all_mons <- c(6)
  ...
  for(i in 1:no_bootstrap_runs) {
    ...
    wdcMerged$tw <- as.ff(rep(0,nrow(wdcMerged)))
    ...
    wdcMerged$tot_demand <- ave(wdcMerged$out_dem_sum*(wdcMerged$stocked_out==F),
                            wdcMerged$groupbyfactor, FUN=sum)
    wdcMerged$tot_obs <- ave(wdcMerged$obs_weight*(wdcMerged$stocked_out==F),
                                wdcMerged$groupbyfactor, FUN=sum)
    wdcMerged$dem_mean <- wdcMerged$tot_demand/wdcMerged$tot_obs
    wdcMerged <- wdcMerged[order(wdcMerged$groupbyfactor,wdcMerged$stocked_out,-wdcMerged$obs_weight),]
    wdcMerged$index <- ave(wdcMerged$station_id_index, wdcMerged$groupbyfactor, FUN=function(x)c(1:length(x)))
        
    wdcMerged <- subset(wdcMerged, index<=1)
    #recreate stockout state to make all 0's except that there are some stations 
    #which are always stocked out, have to make those 1's.
    wdcMerged <- wdcMerged[order(wdcMerged$week_tw, wdcMerged$station_id_index),]
    x <- as.data.table(wdcMerged)
    result <- x[,list(station_id_index,sto_state_local = gen_sto_state_local_char(stocked_out,local_stations)), by = list(week_tw)]
    result <- as.data.frame(result)
    
    wdcMerged$sto_state_local <- NULL
    wdcMerged$sto_state_local_fac <- NULL
    wdcMerged <- merge(wdcMerged, result, by=c("week_tw","station_id_index"))
    wdcMerged$out_dem_sum <- wdcMerged$tot_demand
    wdcMerged$obs_weight <- wdcMerged$tot_obs
