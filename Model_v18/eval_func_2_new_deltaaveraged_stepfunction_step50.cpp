...
double dis_step = 0.05; //threshold for step wise cost for distance
...
vector<mat>  compute_prob(uint i, mat station_data, NumericMatrix xpoints, uint wdclat1_col, uint wdclon1_col,
    uint pointslat1_col, uint pointslon1_col, double beta1, double sigma0, double beta1_step, colvec xdeltain, 
...
vector<mat>  compute_prob_2(uint i, mat station_data, uint wdclat1_col, uint wdclon1_col,
    double pointslat1_i, double pointslon1_i, double beta1, double sigma0, double beta1_step, colvec xdeltain, 
...
//          cart_product(prob_table, outputTemp2, prob_a.begin(), prob_a.end());          
          ...
          for(uint k=0; k <a_vec.size(); ++k) {
            ...
            for(uint l=0; l <a_vec_col.size(); ++l) {
              ...
              pointslon1_col, beta1, sigma0, beta1_step, xdeltain, st_point_list_uvec, deltain_row, mat_st_state_row,  
          ...
          for(uint k=0; k <a_vec.size(); ++k) {
            ...
            for(uint l=0; l <a_vec_col.size(); ++l) {
              ...
              rowvec lambda_st_t = compute_prob_unweighted(i, station_data, wdclat1_col, wdclon1_col, xpoints(i,pointslat1_col), 
                xpoints(i,pointslon1_col), beta1, sigma0, beta1_step, xdeltain, st_point_list_uvec, deltain_row, mat_st_state_row,  
              xv0_vec); 
...
SEXP eval_grad_lambda_theta_cpp_new(SEXP deltain , SEXP  theta1 ,SEXP wdcMergedday , SEXP points,
  ...
  //try {
    ...
    arma::mat grad_t(xwdcMergedday.nrow(),3,fill::zeros);
...
vector<mat>  compute_prob(uint i, mat station_data, NumericMatrix xpoints, uint wdclat1_col, uint wdclon1_col,
    ...
    NumericVector xv0_vec, uint points_density_col) {
          ...
          rowvec station_data_dis_vIdx = station_data_dis_org;
          station_data_dis_vIdx.transform(min_x(dis_step));
          //assign pos(station_data_dis_org-dis_step) to dis_p2
          rowvec station_data_dis_vIdx_step = station_data_dis_org-dis_step;
          station_data_dis_vIdx_step.transform(pos_x());

          rowvec util = exp(beta1*station_data_dis_vIdx + beta1_step*station_data_dis_vIdx_step + deltain_row )% (mat_st_state_row==0);
...
mat  compute_prob_theta(uint i, mat station_data, NumericMatrix xpoints, uint wdclat1_col, uint wdclon1_col,
    ...
    rowvec grad_beta1_step(no_t_st,fill::zeros);
    ...
    //for(int m=0; m<1; ++m) {
        ...
        {
          rowvec disP_step = util_prob_t%station_data_dis_vIdx_step;
          double disP_step_sum = sum( disP_step);
          rowvec disP_step_sum_vec(no_t_st);
          disP_step_sum_vec.fill(disP_step_sum);            
          //#gradient wrt beta1_step is given as dis_step_ij*Pij-Pij(\sum_k dis_step_ik*Pik) for a t
          grad_beta1_step += disP_step - util_prob_t%disP_step_sum_vec;          
        }
    }
    grad_beta1 *= (xpoints(i,points_density_col)/xv0_vec.size());
    grad_sigma0 *= (xpoints(i,points_density_col)/xv0_vec.size());
    grad_beta1_step *= (xpoints(i,points_density_col)/xv0_vec.size());
//    grad_beta1 = grad_beta1 % prob_row;
//    grad_sigma0 = grad_sigma0 % prob_row;
    
    mat obj_ret = join_rows(trans(grad_beta1),join_rows(trans(grad_sigma0),trans(grad_beta1_step))); 
    return(obj_ret);  
...
mat  compute_prob_theta_2(uint i, mat station_data, uint wdclat1_col, uint wdclon1_col,
    ...
    grad_beta1_step *= (pointsden_i/xv0_vec.size());
//    grad_beta1 = grad_beta1 % prob_row;
//    grad_sigma0 = grad_sigma0 % prob_row;
...
SEXP eval_hessian_lambda_theta1_sq_cpp(SEXP deltain , SEXP  theta1 ,SEXP wdcMergedday , SEXP points,
  ...
  //try {
    ...
    uint min_points_col = 2;
    uint max_points_col = xpoints.n_cols-1;
    assert(max_points_col-min_points_col==xtheta1.size()-3); //to assert other points covariates supplied correspond to density vector.
...
mat compute_hessian_theta1_sq_weighted(uint i, mat station_data, uint wdclat1_col, uint wdclon1_col,
    ...
    double point_density_i, rowvec points_den_covariates) {
          ...
          double hessian_beta1_step_sq_t = 0;
          double hessian_beta1_beta1_step_t = 0;
          double hessian_sigma_sq_t=0;
          double hessian_beta1_thetaden_t=0;  
          double hessian_beta1_step_thetaden_t=0;  
          ...
          for(int m=0; m<xv0_vec.size(); ++m) {
              ...
              }
              {
                ...
                double disP_step_sum = sum( disP_step);
                double val1 = util_prob_t(focal_station_index) *\
                  pow(station_data_dis_vIdx_step(focal_station_index)-disP_step_sum,2.0); 
                rowvec disP_step_sum_vec(no_t_st);
                disP_step_sum_vec.fill(disP_step_sum);
                double val2 = util_prob_t(focal_station_index)*sum((station_data_dis_vIdx_step-disP_step_sum_vec) % disP_step);
                hessian_beta1_step_sq_t += val1 - val2;
                hessian_beta1_step_thetaden_t += util_prob_t(focal_station_index)*\
                (station_data_dis_vIdx_step(focal_station_index)-disP_step_sum);
              }
              {
                rowvec disP = util_prob_t%station_data_dis_vIdx;
                double disP_sum = sum( disP);
                ...
                double disP_step_sum = sum( disP_step);
                double val1 = util_prob_t(focal_station_index) *\
                  (station_data_dis_vIdx(focal_station_index)-disP_sum)*(station_data_dis_vIdx_step(focal_station_index)-disP_step_sum);
                rowvec disP_step_sum_vec(no_t_st);
                disP_step_sum_vec.fill(disP_step_sum);
                double val2 = util_prob_t(focal_station_index)*sum((station_data_dis_vIdx_step-disP_step_sum_vec) % disP);

                hessian_beta1_beta1_step_t += val1 - val2;
              }
          ...
          hessian_beta1_step_sq_t *= (1/xv0_vec.size());
          hessian_beta1_step_thetaden_t *= (1/xv0_vec.size());

          vec hessian_beta1_thetaden_vec_t(xtheta1_size-3);
          ...
          vec hessian_beta1_step_thetaden_vec_t(xtheta1_size-3);
          hessian_beta1_step_thetaden_vec_t.fill(hessian_beta1_step_thetaden_t);
          hessian_beta1_step_thetaden_vec_t = hessian_beta1_step_thetaden_vec_t % points_den_covariates.t();

          hessian_theta1(0,0) = hessian_beta1_sq_t * point_density_i;
          hessian_theta1(0,span(3,xtheta1_size-1)) = hessian_beta1_thetaden_vec_t.t();
          hessian_theta1(span(3,xtheta1_size-1),0) = hessian_beta1_thetaden_vec_t;

          hessian_theta1(2,2) = hessian_beta1_step_sq_t * point_density_i;
          hessian_theta1(2,span(3,xtheta1_size-1)) = hessian_beta1_step_thetaden_vec_t.t();
          hessian_theta1(span(3,xtheta1_size-1),2) = hessian_beta1_step_thetaden_vec_t;

          hessian_theta1(0,2) = hessian_beta1_beta1_step_t * point_density_i;
          hessian_theta1(2,0) = hessian_beta1_beta1_step_t * point_density_i;
...
mat compute_hessian_theta1_delta_weighted(uint i, mat station_data, uint wdclat1_col, uint wdclon1_col,
    ...
    double point_density_i, rowvec points_den_covariates) {
          ...
          rowvec hessian_beta1_step_delta_t(no_t_st,fill::zeros);
          ...
          for(int m=0; m<xv0_vec.size(); ++m) {
              ...
              }
              {
                ...
                double disP_step_sum = sum( disP_step);
                rowvec val1(no_t_st,fill::zeros);
                val1 = station_data_dis_vIdx_step; 

                val1 += station_data_dis_vIdx_step(focal_station_index) - 2*disP_step_sum;
                
                val1 = val1 % util_prob_t;
                val1 *= -util_prob_t(focal_station_index);
                //remove focal_station_index from val1 as it is incorrect.
                val1.shed_col(focal_station_index);
                hessian_beta1_step_delta_t(no_focal_indexes) += val1;
                hessian_beta1_step_delta_t(focal_station_index) += util_prob_t(focal_station_index) * (1-2*util_prob_t(focal_station_index))*\
                  (station_data_dis_vIdx_step(focal_station_index)-disP_step_sum); 
              }
          ...
          hessian_beta1_step_delta_t *= (1/xv0_vec.size())* point_density_i;
          ...
          hessian_theta1_delta_t.row(2)=hessian_beta1_step_delta_t;
          hessian_theta1_delta_t.rows(span(3,xtheta1_size-1))=hessian_thetaden_delta_t;
...
SEXP eval_grad_lambda_cpp_new(SEXP deltain , SEXP  theta1 ,SEXP wdcMergedday , SEXP points,
  ...
  //try {
    ...
    arma::mat grad_nondensity_cols(xwdcMergedday.nrow(),3,fill::zeros);
