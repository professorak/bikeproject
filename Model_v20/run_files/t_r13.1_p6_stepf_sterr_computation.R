...
theta <- c(-33.68060527, -475.89123847,    0.22374614,  289.74220127,    1.96685472,
           277.99071352,    1.01709297,    0.05885138,    0.00000000)
...
##step 2
theta <- c(-57.1227617, -116.9732969,    0.6170171,  436.1557314,    0.9318717,
           869.5232115,    4.7688933,    0.1022040,    0.0000000)
#choose starting values for deltain_stin
#delta_all <- rnorm(nrow(wdcMerged),-10,1)
delta_all <- compute_delta_list_cntrt_map_new(c(theta[1],0,theta[-1]),wdcMerged, points)
deltain_stin <- delta_all[which(wdcMerged$stocked_out==F)]
