...
sink("r9.6_1:10_full_stepf_pr6.txt",split=T)
...
sourceCpp("eval_func_2_new_deltaaveraged_stepfunction_steps_set2.cpp")
...
source("eval_covariates_delta_reg_step_servlvl_moredensitycols.R")
...
colnames_theta1 <<- c("dis coef","rand coef","dis coef","dis coef","dis coef",
                      ...
                      "density metro evening", "density grocery_or_supermarket", "local_government_office", "food",
                      "density bus")
...
density_google_places_theta_cols <- c(10,11,12)
density_google_places_points_cols <- c(9,10,11)


weighing_GMM_mat <<- NULL

length_stklist <- length(which(wdcMerged$stocked_out==F))
theta <- c(-4 ,-4,-4,-4, 1, 146.83, 20, 203.69, 0.42, 0.42,0.42,9.41)
...
lb = c(-rep(1000,4),0,rep(0,7),rep(-20, length_delta), rep(-1.0e19,length_eta))
ub = c(rep(1000,4),1000000,rep(1000000,7),rep(10, length_delta), rep(1.0e19,length_eta))
...
active_coef <- c(1:length_theta)
active_coef_nointercept <- c(1:length_theta)[-c(density_intercept_col-1)]
...
print("theta2 t-stat:")
print(round(theta2_save[serv_cols]/sqrt(diag(var_covar_theta_2)[serv_cols]),2))
