...
sink("rb24.1_1:10_full_stepf_pr6.txt",split=T)
...
source("eval_covariates_delta_reg_densitycols_setb24.R") #all googleplaces etc. density variables plus census density
#and NO catchement area instruments + service level single step
...
theta <- c(-4 ,-4, 1, 0, 20, 0, rep(0.42, 5),0,1)

active_coef <- c(1:length(theta))[-c(density_add_den_cols[1]-1,density_metro_evening_col-1,density_ridership_col-1,density_metro_col-1)]
active_coef_nointercept <- c(1:length(theta))[-c(density_intercept_col-1,density_add_den_cols[1]-1,density_metro_evening_col-1,
                                                 density_ridership_col-1,density_metro_col-1)]
...
ub = c(rep(300,2),
       1000000,0,rep(1000000,1),0,rep(1000000,5),0,1000000,
