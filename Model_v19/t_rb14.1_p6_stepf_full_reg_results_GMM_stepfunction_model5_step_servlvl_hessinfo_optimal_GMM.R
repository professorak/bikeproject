...
sink("rb14.1_1:10_full_stepf_pr6.txt",split=T)
...
source("eval_covariates_delta_reg_densitycols_setb14.R") 
#all metro, googleplaces etc. density variables + census density
...
theta <- c(-4 ,-4, 1, 146.83, 20, 0, 0.42, 0.42,0,1)

active_coef <- c(1:length(theta))[-c(density_add_den_cols[1]-1,density_metro_evening_col-1)]
...
lb = c(-rep(300,2),0,rep(0,5),0,0,rep(-20, length_delta), rep(-1.0e19,length_eta))
ub = c(rep(300,2),1000000,rep(1000000,2),0,rep(1000000,2),0,1000000,rep(10, length_delta), rep(1.0e19,length_eta))
