...
sink("rb8.1_1:10_full_stepf_pr6.txt",split=T)
...
#convert metro variables to dummies
points$weight[which(points$type==2)] <- 1
#convert metro moment variables to dummies
list_vars <- which(colnames(wdcMerged) %like% "metro")
print(colnames(wdcMerged)[list_vars])
for(i in list_vars) {
  wdcMerged[,i] <- as.numeric(wdcMerged[,i]!=0)  
}
...
source("eval_covariates_delta_reg_densitycols_setb7.R") #all metro, googleplaces etc. density variables minus census density
...
theta <- c(-4 ,-4, 0, 146.83, 20, 0, 0.42, 0.42,0)

active_coef <- c(1:length(theta))[-c(density_bus_col-1,density_metro_evening_col-1,density_ridership_col-1)]
...
ub = c(rep(300,2),0,rep(1000000,2),0,rep(1000000,2),0,rep(10, length_delta), rep(1.0e19,length_eta))
