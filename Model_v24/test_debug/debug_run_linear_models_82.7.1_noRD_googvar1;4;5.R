...
source("eval_covariates_linear_reg_googvar1,4,5.R")
...
density_vars <- colnames(X)[c(which(colnames(X) %like% "census_density"),
                  ...
                  which(colnames(X) %like% "googleplaces_museum_den"),
                  which(colnames(X) %like% "googleplaces_movie_theater_den"),
                  which(colnames(X) %like% "googleplaces_government_den"))]
...
#removing obs with wdcMerged$out_dem_mean==0
idx0 <- which(wdcMerged$out_dem_mean==0.00001)
sum(wdcMerged$obs_weight[idx0])/sum(wdcMerged$obs_weight)*100
wdcMerged_0 <- wdcMerged[-idx0,]
X_0 <- X[-idx0,]
...
coeftest(fit, vcov = vcovHC(fit, "HC1"))    # robust; HC1 (Stata default)
...
fit <- ivreg( log(wdcMerged_0$out_dem_mean) ~ 
                X_0[,"Intercept"] +  X_0[,other_vars] + 
                X_0[,tract_tw_vars] + X_0[,distance_vars] + X_0[,density_vars]
              + Instr_df[-idx0,c("serv_lvl","serv_lvl_sq")] + 0 |                 
                X_0[,"Intercept"] +  X_0[,other_vars] + 
                X_0[,tract_tw_vars] + X_0[,distance_vars] + X_0[,density_vars]
              + Instr_df[-idx0,c("in_dem_rate_lagtw","in_dem_rate_lagtw_sq")] + 0, 
              weights=wdcMerged_0$obs_weight)
summary(fit)
