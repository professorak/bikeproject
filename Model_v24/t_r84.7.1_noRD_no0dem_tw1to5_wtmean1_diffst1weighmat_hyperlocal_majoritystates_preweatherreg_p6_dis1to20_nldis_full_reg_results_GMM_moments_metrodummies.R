...
sink("r84.7.1_noRD_no0dem_tw1to5_wtmean1_diffst1weighmat_hyperlocal_majoritystates_preweatherreg_metrodummies_moments_1:20_full_nldis_pr6.txt",split=T)
...
source("GetDataGooglePlaces_20dis_aggmonth_hyperlocalstate_majoritystates_preweatherreg_moreinstr_withreallocthresh_stkoutthresh_five_averaged_pr6_no0dem.R")

#setting obs_weight mean to 1
wdcMerged$obs_weight <- wdcMerged$obs_weight/mean(wdcMerged$obs_weight)
...
active_coef <- c(1:length(theta1))[-c(2,density_google_places_theta_cols,
                                      density_add_den_cols[1],metro_on_idx,metro_off_idx)]
active_coef_nointercept <- c(1:length(theta1))[-c(2,density_intercept_col,
                                                  density_google_places_theta_cols,density_add_den_cols[1],metro_on_idx,metro_off_idx)]
...
Z_weighted <- (Z * wdcMerged$obs_weight[stocked_list])
S0 = (t(Z_weighted) %*% Z_weighted) / sum(wdcMerged$obs_weight[stocked_list])
print("kappa(t(Z_weighted) %*% Z_weighted): ")
print(kappa(S0))
weighing_GMM_mat <- solve(S0)
