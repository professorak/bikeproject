...
eval_constraints_moment_conditions <- function(deltain, theta1, eta, wdcMerged, points) {  
  ...
  if(print_iter_values) {
    paste0("serv_lvl coef: ",print(theta_2[which(rownames(theta_2) %like% "serv_lvl")]/scalecols_X[which(rownames(theta_2) %like% "serv_lvl")]))
  }
...
eval_obj_GMM_MPEC <- function(theta1, deltain, wdcMerged, points) {  
  ptm <- proc.time()
  ...
  print(theta)
  print("gradient:")
  print(c(gmm_gradient[1:length(theta1)],mean(gmm_gradient[-c(1:length(theta1))])))
  print("eval_obj time:")
  print(proc.time()-ptm)
