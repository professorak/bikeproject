...
#add serv_lvl to points
# - sl1-- Average service level of stations within 300 mts. (If there are fewer stations than 3, then this will take nearest 3 stations’ average)
# - sl2-- Average service level of nearest 3 stations.
# - sl3-- Average service level of stations within 150 mts. (If there are fewer stations than 3, then this will take nearest 3 stations’ average)

tw_list <- sort(unique(wdcMerged$tw))

points$local_stations <- rep(NA,nrow(points))
points$sl1_stations <- rep(NA,nrow(points))
points$sl2_stations <- rep(NA,nrow(points))
points$sl3_stations <- rep(NA,nrow(points))
sl1_cols <- paste("sl1_",tw_list, sep="")
points[,sl1_cols] <- rep(NA,nrow(points))
sl2_cols <- paste("sl2_",tw_list, sep="")
points[,sl2_cols] <- rep(NA,nrow(points))
sl3_cols <- paste("sl3_",tw_list, sep="")
points[,sl3_cols] <- rep(NA,nrow(points))
...
for(i in 1:nrow(points)) {
  ...
  # - sl1-- Average service level of stations within 300 mts. (If there are fewer stations than 3, then this will take nearest 3 stations’ average)  
  sl1_stations <- station_data$station_id_index[sort(union(which(dis_v <= 0.300), order_dis_v[1:3]))]
  points$sl1_stations[i] = paste(sl1_stations, collapse="_")  
  points[i,sl1_cols] <- get_sl_alltw(sl1_stations)
  
  # - sl2-- Average service level of nearest 3 stations.
  sl2_stations <- station_data$station_id_index[sort(order_dis_v[1:3])]
  points$sl2_stations[i] = paste(sl2_stations, collapse="_")  
  points[i,sl2_cols] <- get_sl_alltw(sl2_stations)
  
  # - sl3-- Average service level of stations within 150 mts. (If there are fewer stations than 3, then this will take nearest 3 stations’ average)  
  sl3_stations <- station_data$station_id_index[sort(union(which(dis_v <= 0.150), order_dis_v[1:3]))]
  points$sl3_stations[i] = paste(sl3_stations, collapse="_")  
  points[i,sl3_cols] <- get_sl_alltw(sl3_stations)
...
###save commands
# save(wdcMerged, file="wdcMerged_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr6.RData")
# save(current_serv_lvl, file="current_serv_lvl_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr6.RData")
# save(user_serv_lvl, file="user_serv_lvl_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr6.RData")
# save(points, file="points_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr6.RData")

# save(wdcMerged, file="wdcMerged_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr3.RData")
# save(current_serv_lvl, file="current_serv_lvl_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr3.RData")
# save(user_serv_lvl, file="user_serv_lvl_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr3.RData")
# save(points, file="points_lin_dis1to20_aggmonths_bytw_hyperlocalstate_withreallocthresh_preweatherreg_stkoutthresh_five_pr3.RData")
