...
//          cart_product(prob_table, outputTemp2, prob_a.begin(), prob_a.end());          
          ...
          for(uint k=0; k <a_vec.size(); ++k) {
            ...
            for(uint l=0; l <a_vec_col.size(); ++l) {
              ...
              pointslon1_col, beta1_vec, sigma0, xdeltain, st_point_list_uvec, deltain_row, mat_st_state_row,  
          ...
          for(uint k=0; k <a_vec.size(); ++k) {
            ...
            for(uint l=0; l <a_vec_col.size(); ++l) {
              ...
              rowvec lambda_st_t = compute_prob_unweighted(i, station_data, wdclat1_col, wdclon1_col, xpoints(i,pointslat1_col), 
                xpoints(i,pointslon1_col), beta1_vec, sigma0, xdeltain, st_point_list_uvec, deltain_row, mat_st_state_row,  
              xv0_vec); 
...
vector<mat>  compute_prob(uint i, mat station_data, NumericMatrix xpoints, uint wdclat1_col, uint wdclon1_col,
    ...
    NumericVector xv0_vec, uint points_density_col) {
          ...
          rowvec util = exp(beta1_vec * station_data_dis_step_mat + deltain_row )% (mat_st_state_row==0);
    ...
    //for(int m=0; m<1; ++m) {
        ...
        rowvec util_prob_t =  util/denutil_t;        
        {
          ...
          for(uint i=0; i< station_data_dis_step_mat.n_rows; ++i) {
            ...
            grad_beta1_vec.row(i) += disP - util_prob_t%disP_sum_vec;
          }
        }
    }
    ...
    double point_density_i, rowvec points_den_covariates) {
          ...
          for(int m=0; m<xv0_vec.size(); ++m) {
...

              {
                ...
                hessian_beta1_vec +=  val1_mat - val2_mat;    

                hessian_beta1_vec_thetaden_t += util_prob_t(focal_station_index)*\
                  (station_data_dis_step_mat.col(focal_station_index) - disP_mat_sum);

                double prob0 = 1-sum( util_prob_t); 
                hessian_beta1_vec_sigma0 += (station_data_dis_step_mat.col(focal_station_index) - 2 * disP_mat_sum) *\
                  util_prob_t(focal_station_index) * prob0 * xv0_vec(m);

                hessian_sigma0_sq += util_prob_t(focal_station_index) * prob0 * (2 * prob0 - 1) * xv0_vec(m) * xv0_vec(m);

                hessian_sigma0_thetaden_t += util_prob_t(focal_station_index) * prob0 * xv0_vec(m);
              }
          }

          hessian_beta1_vec *= (1/xv0_vec.size());
          hessian_beta1_vec_thetaden_t *= (1/xv0_vec.size());
          hessian_beta1_vec_sigma0 *= (1/xv0_vec.size());
          hessian_sigma0_sq *= (1/xv0_vec.size());
          hessian_sigma0_thetaden_t *= (1/xv0_vec.size());
...
mat compute_hessian_theta1_delta_weighted(uint i, mat station_data, uint wdclat1_col, uint wdclon1_col,
    ...
    double point_density_i, rowvec points_den_covariates) {
          ...
          for(int m=0; m<xv0_vec.size(); ++m) {
              ...
              rowvec util_prob_t =  util/denutil_t;              
              {
                for(uint i=0; i< station_data_dis_step_mat.n_rows; ++i) {
                  ...
                  uvec row_i; row_i << i;                  
                  hessian_beta1_vec_delta_t(row_i,no_focal_indexes) += val1;
                  hessian_beta1_vec_delta_t(i,focal_station_index) += util_prob_t(focal_station_index) * (1-2*util_prob_t(focal_station_index))*\
                    (station_data_dis_step_mat(i,focal_station_index)-disP_sum);
                }
                //for sigma0_delta  
                double prob0 = 1-sum( util_prob_t);
                rowvec val2(no_t_st,fill::zeros);
                val2 = util_prob_t % util_prob_t;
                val2 *= -2 * prob0 * xv0_vec(m);
                hessian_sigma0_delta_t += val2;
                hessian_sigma0_delta_t(focal_station_index) += util_prob_t(focal_station_index) * prob0 * xv0_vec(m);
              }
          ...
          hessian_beta1_vec_delta_t *= (1/xv0_vec.size())* point_density_i;
          hessian_sigma0_delta_t *= (1/xv0_vec.size())* point_density_i;
